...
#eigenvectors for spin-half operators
...
def run_two_measurements():
    ...
    #simualte how spins deals with blocked paths
    for i in range(N): #for every atom in N
        ...
        if outcome1 == filter_choice: #if outcome of axis1 is the same as the filter choice, outcome passes through, otherwise blocked (nothing happens)
            _, outcome2 = measure(collapsed_state, axis2)  #measure the atoms that passed through along the second axis, outcome2 is "up" or "down" based on quantum probabilities
            if outcome2 == "up":
                second_up += 1
            else:
                second_down += 1
